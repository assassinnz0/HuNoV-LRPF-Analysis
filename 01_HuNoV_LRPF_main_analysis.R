# HuNoV-LRPF publication analysis (version 10.3)
#
# This script reproduces the complete manuscript analysis:
#   1. development on the five-inoculum panel;
#   2. fixed-settings analysis of GZJY122;
#   3. fixed-specification six-inoculum update;
#   4. manuscript and supplementary figures, source tables, model objects,
...
data_dir <- NULL
output_root <- file.path(project_dir, "HuNoV_LRPF_v10_3_full_results")
...
# =============================================================================
# 6. Fixed-settings analysis of GZJY122
# =============================================================================
...
main_figures <- list(
  ...
  Figure_5_GZJY122_fixed_settings_analysis = list(
...
table_1_dataset_roles <- dplyr::bind_rows(
  ...
  application_file |>
    dplyr::transmute(
      ...
      Purpose = "Fixed-settings application"
    )
)
...
expected_main_columns <- list(
  ...
  Table_2_RT_qPCR_primers_probes = c(
    "Norovirus-GII", "Name", "Sequence（5ʹ–3ʹ）", "Reference"
  ),
...
write_run_metadata(
  ...
  version = "10.3-bw-figures-v93-figure4-table-aligned"
)
write_output_manifest(output_root)

message("HuNoV-LRPF full publication analysis completed: ", normalizePath(output_root, winslash = "/", mustWork = TRUE))
